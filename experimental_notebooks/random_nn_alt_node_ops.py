...
def RandDenseNodeOps(prev_layer, out_shape, layer_count_range, layer_size_range, param_lim, high_skips=False, relu_position='before', weighted_sum=True):
    ...
    if n_neurons > n_in:
        if relu_position == 'before':
            chan_pad = relu(prev_layer)
            chan_pad = Dense(n_neurons-n_in)(chan_pad)
        elif relu_position == 'after':
            chan_pad = Dense(n_neurons-n_in, activation='relu')(prev_layer)
...
def RandDenseBN(prev_layer, out_shape, layer_count_range, layer_size_range, param_lim, high_skips=False):
    # prev_layer is expected to have shape (None, x)
    n_in = prev_layer.shape[1]
    n_out = out_shape
    ...
    if n_neurons > n_in:
        chan_pad = relu(prev_layer)
        chan_pad = Dense(n_neurons-n_in)(chan_pad)
        chan_pad = BatchNormalization()(chan_pad)
        layers = [concatenate([prev_layer, chan_pad])]
    else:
        activation = relu(prev_layer)
        dense = Dense(n_neurons)(activation)
        layers = [BatchNormalization()(chan_pad)]
...
def RandDenseLN(prev_layer, out_shape, layer_count_range, layer_size_range, param_lim, high_skips=False):
    # prev_layer is expected to have shape (None, x)
    n_in = prev_layer.shape[1]
    n_out = out_shape
    ...
    if n_neurons > n_in:
        chan_pad = relu(prev_layer)
        chan_pad = Dense(n_neurons-n_in)(chan_pad)
        chan_pad = LayerNormalization()(chan_pad)
        layers = [concatenate([prev_layer, chan_pad])]
    else:
        activation = relu(prev_layer)
        dense = Dense(n_neurons)(activation)
        layers = [LayerNormalization()(chan_pad)]
